...
//비동기 방식으로 상위 카테고리 요청하기!!
function getTopList(obj){
   //alert($(obj).val());
   
   $.ajax({
      url:"/cinderella/admin/product/toplist", 
      type:"get",
      data:{
         gender_id:$(obj).val()         
      },
      success:function(result){
         ...
         //기존 option태그를 먼저 지우자!!
         $($("select")[1]).empty();
         $($("select")[1]).append("<option>상위 카테고리 선택</option>");
         for(var i=0;i<result.length;i++){
            var topCategory = result[i]; //subcategory 1건에 대한 json 객체얻기!!
            $($("select")[1]).append("<option value=\""+topCategory.topcategory_id+"\">"+topCategory.topcategory_name+"</option>");
         }
      }
   });
}
...
function getSubList(obj){
      ...
      $.ajax({
         ...
         success:function(result){
            ...
            //기존 option태그를 먼저 지우자!!
            $($("select")[2]).empty();
            $($("select")[2]).append("<option>하위 카테고리 선택</option>");
            
            for(var i=0;i<=result.length;i++){
               var subCategory = result[i]; //subcategory 1건에 대한 json 객체얻기!!
               $($("select")[2]).append("<option value=\""+subCategory.subcategory_id+"\">"+subCategory.subcategory_name+"</option>");
            }
         }
      });
   }
...
<div class="container">
  <form>
     ...
     <select>
        <option>성별 선택</option>
        <%for(GenderCategory genderCategory : genderList){%>
        <option value="<%=genderCategory.getGender_id() %>"><%=genderCategory.getGender_name() %></option>
        <%} %>
     </select>
    ...
    <input type="text" name="quantity" placeholder="대여 수량">
